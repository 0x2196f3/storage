...
# ---------------------------------------------------------------
prism1 = Part.makeBox(5, height, width)         # (dx, dy, dz)
...
# ---------------------------------------------------------------
outer_half = Part.makeCylinder(13, width, App.Vector(0, 0, 0),
                               App.Vector(0, 0, -1), 150)
inner_half = Part.makeCylinder(8, width, App.Vector(0, 0, 0),
                               App.Vector(0, 0, -1), 150)
...
shell.translate(App.Vector(-8, 0, width))
...
# ---------------------------------------------------------------
prism2 = Part.makeBox(75, 5, width)
...
full_cyl  = Part.makeCylinder(radius, width, centre, App.Vector(0, 0, 1))
# remove the part where X < 0 to obtain a semicircle lying on X ≥ 0
cut_box   = Part.makeBox(radius, radius*2, width,
